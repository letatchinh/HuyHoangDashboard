...
import { Button, Collapse, Flex, Popconfirm, Spin } from "antd";
...
export default function ScheduleList(props: propsType): React.JSX.Element {
  ...
  const dataSource = useMemo(
    () =>
      data?.map((item: ScheduleBase) => ({
        key: item?._id,
        label: item?.name || "",
        children: [
          <ScheduleItemList
            dataSource={item?.scheduleItem || []}
            scheduleId={item._id}
          />,
        ],
        extra: isSubmitLoading ? (
          <Spin spinning />
        ) : (
          <Flex gap={10}>
            <Button
              size="small"
              type="text"
              onClick={(e) => handleUpdateSchedule(e, item?._id)}
              icon={
                <EditOutlined
                />
              }
