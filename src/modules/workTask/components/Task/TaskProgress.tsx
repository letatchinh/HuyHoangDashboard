import { Button, Checkbox, Col, Dropdown, Image, List, Modal, Row, Tag, Tooltip, Upload, notification, UploadFile } from 'antd'
import React, { createContext, useCallback, useContext, useEffect, useState , Dispatch, SetStateAction } from 'react'
import { CopyOutlined, DeleteOutlined, DownloadOutlined, LinkOutlined, UserAddOutlined } from '@ant-design/icons'
...
export const useProgressContext = ()=>useContext<PropsContext>(ProgressListItemContext);

function TaskProgress({item,progress,canAssign,index,onUpdateProgress,dataTask,onAddProcess}: Props) {
      ...
      <List.Item
        ...
        actions={[
          <Tooltip title={'Tải tệp lên'} mouseEnterDelay={0.8}>
             <Button 
              onClick={() =>
                setVisibilityUpload((e)=>!e as any)
              }
              ...
              <Col span={24} style={{ height: 'max-content' }}>
                <Upload
                  ...
                  itemRender={(OriginNode, file, currFileList,v) => {
                    return (
                      <div key={file.url } style={{position:'relative' ,height:'100%'}}>
                       {  OriginNode  }
                        <a style={{position:'absolute' ,bottom:3, right:3, padding:'4px 6px', fontSize:12, zIndex:999, backgroundColor:'#3E80F3', color:'white', borderRadius:'50% 0 0 0'}} href={file.url} target="_blank" download={file.name}>
                          <DownloadOutlined />
                        </a>
                      </div>
                    );
...
const Content = ({ item, index, onUpdateProgress, progress }: PropsContent) => {
  ...
  const onUpdate = () => {
    onUpdateProgress(get(progress, '_id'), index, [get(item, '[0]'), { ...get(item, '[1]'), content: text }])
    onCancel()
  }

  return isOpen ?
    <div>
      <TextArea style={{ width: '100%' }} rows={4} autoFocus placeholder='Nhập nội dung công việc' value={text} onChange={(e) => setText(e.target.value)} />
    ...
    : <Text style={{ cursor: 'pointer' }} onClick={toggle}>{get(item, '[1].content')}</Text>
}
