...
import { useGetTaskById, useResetComment, useUpdateProgress, useUpdateTask } from '../workTask.hook'
...
export default function TaskItem({ idTask }: Props) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
   ...
   useEffect(() => {
    const statusItem = boardById?.listStatusConfig.find((item: any) => item?._id === dataTask?.statusId?._id);
  ...
  const handlFinshed = useCallback((val : any, key : any) => {
    updateTask({
      [key]: val,
      id: idTask || id
    })
