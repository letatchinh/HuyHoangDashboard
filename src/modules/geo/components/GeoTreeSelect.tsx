...
import { compact, get, head, sortBy, split, unionBy } from 'lodash';
...
const GeoTreeSelect = ({
  ...
  const wards = useMemo(() => subvn.getWardsByDistrictCode(selectedDistrictCode),[selectedDistrictCode])
  const districts = useMemo(() => subvn.getDistrictsByProvinceCode(selectedCityCode),[selectedCityCode])
  ...
  useEffect(() => {
    ...
    const newTree : any = unionBy(treeData, areas, 'code')
      ...
      return {
        ...
        unit: "area",
        checkable: checkablePositions.includes(position) || enableToSelectAllEnabledValues,
        selectable: checkablePositions.includes(position),
        disabled: !checkablePositions.includes(position) && !enableToSelectAllEnabledValues,

        children: unionBy(
          get(treeData, [a, 'children'].join('.')) || [],
          cities,
          'code'
        ).filter(city => parseInt(get(city,'area_code','')) === parseInt(code) && getShouldShow(buildPath([kA, get(city,'code')])))
          .map((city : any, c) => {
            const { code, name } = city;
            const kC = code;
            const pathC = '/'.concat([kA, kC].join('/'))
            const parentSelected = valuePaths?.some((item : any) => pathC?.length > item?.length && pathC.startsWith(item))
            ...
            return {
              ...
              title: name,
              unit: "city",
              checkable: !parentSelected && (checkablePositions.includes(position) || enableToSelectAllEnabledValues),
              selectable: !parentSelected && checkablePositions.includes(position),
              disabled: parentSelected || (!checkablePositions.includes(position) && !enableToSelectAllEnabledValues),
              children: unionBy(
                get(treeData, [a, 'children', c, 'children'].join('.')) || [],
                districts,
                'code'
              ).filter(district => parseInt(get(district,'province_code','')) === parseInt(code) && getShouldShow(buildPath([kA, kC, get(district,'code')])))
                .map((district : any, d) => {
                  const { code, name } = district;
                  const kD = code;
                  const pathD = '/'.concat([kA, kC, kD].join('/'))
                  const parentSelected = valuePaths?.some((item : any) => pathD?.length > item?.length && pathD.startsWith(item))
                  ...
                  return {
                    ...
                    children: unionBy(
                      get(treeData, [a, 'children', c, 'children', d, 'children'].join('.')) || [],
                      wards,
                      'code'
                    ).filter((ward : any) => parseInt(ward.district_code) === parseInt(code) && getShouldShow(buildPath([kA, kC, kD, ward.code])))
                      .map((ward : any, w) => {
                        console.log(ward,'ward')
                        const { code, name } = ward;
                        const kW = code;
                        const pathW = '/'.concat([kA, kC, kD, kW].join('/'))
                        const parentSelected = valuePaths?.some((item : any) => pathW?.length > item?.length && pathW.startsWith(item))
                        ...
                        return {
                          ...
                          title: name,
                          unit: "ward",
                          checkable: !parentSelected && checkablePositions.includes(position),
                          selectable: !parentSelected && checkablePositions.includes(position),
                          disabled: parentSelected || !checkablePositions.includes(position),
                        }
                      })
                  }
                })
  ...
  const onTreeSelectChange = (value : any) => {
    ...
    if (get(restProps, "multiple")) {
      ...
      if (get(restProps, "labelInValue")) {
        ...
        const reducedValue = sortedValue.reduce((prev, curr) => {
          ...
          const isCurrentChildOfCurrentParent = get(curr, "value").startsWith(get(currentParent, "value"));
